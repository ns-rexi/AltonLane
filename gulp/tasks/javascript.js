...
function processJavascript(cb)
{
	...
	,	function(config, cb)
		{
			//this is to build the SCL distribution (yo n:release)
...
gulp.task('javascript', ['templates', 'javascript-entrypoints'], processJavascript);
...
gulp.task('watch-javascript', ['watch-templates'], function()
{
	...
	{
		gulp.watch(path.join(process.gulp_dest, 'processed-templates/*'), {interval: 2000}, ['javascript-no-deps']);
	}
	else
	{
		gulp.watch(package_manager.getGlobsFor('javascript'), {interval: 2000}, ['javascript-no-deps']);
	}
});
