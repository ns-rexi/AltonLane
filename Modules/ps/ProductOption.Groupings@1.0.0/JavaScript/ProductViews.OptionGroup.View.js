define('ProductViews.OptionGroup.View', [
    'SC.Configuration',
    'Backbone',
    'underscore',
    'jQuery',
    'bootstrap-datepicker'
], function ProductViewsOptionGroupView(
    Configuration,
    Backbone,
    _,
    jQuery
    ...
    return Backbone.View.extend({
        ...
        events: {
            'click div[class*="monogram"]': 'monogramUpdate',
            'click div[class*="mongram"]': 'monogramUpdate'
        },
        monogramUpdate: function monogramUpdate(e) {
            jQuery(e.target).parents('div[data-validation="control-group"]').find('.data-validation').remove();
        },
        getContext: function () {
            var self = this;
            var img_help;
            var img_help_val = this.model.get('helpImages');
            if (img_help_val)
                img_help = img_help_val.split('|');

            var selected_value = this.model.get('value') || {};
            var i = 0;
            var values = _.map(this.model.get('values'), function (value) {
                var color = '';
                var is_color_tile = true;
                var image = {};
                var help_option_image = '';
                if (img_help)
                    help_option_image = img_help[i] || '';
                else
                    help_option_image = ''
                ...
                return {
                    ...
                    , help_option_image: help_option_image
                };
            ...
            return {
                ...
                , maxLength: this.model.get('maxlength') ? ' maxlength=' + this.model.get('maxlength') : ''
            };
            // @class ProductViews.Option.View
        }
    });
